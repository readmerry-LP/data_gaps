...
whs_gap_type_weekly_d = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
for r in records:
    if r['warehouse']:
        ...
        _gt = r['gap_col'].replace(' Gap Indicator','').replace(' Indicator','')
        whs_gap_type_weekly_d[r['warehouse']][r['week']][_gt] += 1

whs_new_vs_co_d = {}
for whs in whs_ranked:
    whs_new_vs_co_d[whs] = {}
    for i, w in enumerate(all_weeks):
        curr = whs_weekly_doc_d[whs].get(w, set())
        prev = whs_weekly_doc_d[whs].get(all_weeks[i-1], set()) if i > 0 else set()
        co = len(curr & prev)
        whs_new_vs_co_d[whs][w] = {'new': max(0, len(curr) - co), 'carryover': co}
...
_whs_gap_type_weekly = dict()
for whs in whs_ranked:
    _whs_gap_type_weekly[whs] = {}
    for w in all_weeks:
        _whs_gap_type_weekly[whs][w] = dict(whs_gap_type_weekly_d[whs].get(w, {}))
...
data = dict(
    ...
    whs_gap_types       = _whs_gap_types,
        whs_new_vs_co       = whs_new_vs_co_d,
        whs_gap_type_weekly = _whs_gap_type_weekly,
